...
<<<<<<< HEAD
        ...
        -   `型式` (D22): マクロが旧ブックを開き、`D8`のアドレスから取得した値を表示する。
        -   `種類` (D23): `D22`の値を元に、人が識別しやすい形で表示される種類名（メモ用）。
        -   `新ブック_ファイルパス` (D24): `D22`の「型式」をキーに`List`シートを検索し、該当するテンプレートパスを自動で表示する。
=======
...
>>>>>>> 4a358f29dfa0286bcc247843225878f0af2a21d0
    ...
    -   **ステップ3：処理内容の定義 (A51以降)**
        -   1行を1つの処理ステップとし、マクロはここに定義された順序でデータ移行処理を実行する（開始行を34行目から51行目に変更）。
        -   各行は以下の列で構成される。
...
本ツールは、動的にログ出力を制御する機能を提供する。

-   **2.6.1. ログ制御変数**
    -   `ENABLE_ERROR_LOG`: ErrorLogシートへの出力制御（True=有効, False=無効）
    -   `ENABLE_DEBUG_LOG`: DebugLogの出力制御（True=有効, False=無効）
    -   デフォルトは両方とも`True`（有効）

-   **2.6.2. ログ種別**
    -   `Log`シート: 主要な処理の進行状況を記録
    -   `ErrorLog`シート: エラー発生時の詳細情報を記録
    -   `DebugLog`シート: 詳細なデバッグ情報を記録
